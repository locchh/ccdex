# ccdex

> A curated collection of resources, tools, and references for building with Claude Code.

## Related Resources

- [Claude Blog](https://claude.com/blog) — Official blog with updates and insights

- [Claude Code Docs](https://code.claude.com/docs/en/overview) — Official Claude Code documentation and overview

- [Official Anthropic Plugins](https://github.com/anthropics/claude-plugins-official) — Official plugin repository by Anthropic

- [Claude Code Actions](https://github.com/anthropics/claude-code-action) — Claude Code Actions

- [Official Anthropic Skills](https://github.com/anthropics/skills) — The Swiss Army knife everyone talks about. Brainstorming, debugging, TDD enforcement, execution planning - all with slash commands.

- [Awesome Claude Code](https://awesomeclaude.ai/awesome-claude-code) — A curated list of awesome Claude Code resources

- [30 Community Claude Skills Tested for a Week](https://www.reddit.com/r/ClaudeAI/comments/1ok9v3d/i_tested_30_community_claude_skills_for_a_week/) — Real-world skill reviews from the community

- [Claude Code Tips from 6 Months of Use](https://www.reddit.com/r/ClaudeAI/comments/1oivjvm/claude_code_is_a_beast_tips_from_6_months_of/) — Practical tips and workflows from power users

- [How to Build a Coding Agent](https://github.com/ghuntley/how-to-build-a-coding-agent) — A comprehensive guide to building coding agents

- [Loom](https://github.com/ghuntley/loom) — Loom is an AI-powered coding agent built in Rust. It provides a REPL interface for interacting with LLM-powered agents that can execute tools to perform file system operations, code analysis, and other development tasks.
